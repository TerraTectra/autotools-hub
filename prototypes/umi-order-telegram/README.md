...
Проверяемый модуль UMI.CMS: уведомления в Telegram при изменении статуса заказа.

## Возможности

- штатное событие `order-status-changed`;
- административная форма настроек UMI.CMS;
...
- автономные unit-тесты и preflight-проверка.
...
Путь вводится в `/admin/config/modules`. После установки открыть модуль «Telegram: статусы заказов» и заполнить настройки.
...
- токен Telegram-бота;
- ID чата или канала;
- ID отслеживаемых статусов через запятую;
- опциональная передача контактов покупателя;
- timeout от 1 до 15 секунд;
- домен сайта для административной ссылки.

Сохранённый токен не выводится обратно в HTML формы. Пустое поле при повторном сохранении оставляет текущее значение без изменений. Контакты покупателя отключены по умолчанию.
...
Версия для UMI.Market после проверки на NFR-контуре должна получить официальный tar-пакет через экспортёр UMI.CMS и пройти модерацию.

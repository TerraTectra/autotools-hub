...
MODULE_PATH = Path(__file__).resolve().parents[1] / "scripts" / "freelance_ru_scan.py"
SPEC = importlib.util.spec_from_file_location("freelance_ru_scan", MODULE_PATH)
...
def sample(marker: str = "Видно всем", title: str = "Python API bot", applications: int = 4, age: str = "2 часа назад", body: str = "Нужно исправить API и тесты") -> str:
    ...
    <html><body>
      ...
      <p>{body}</p>
      <div>{age}</div><div>Не важна</div><div>{applications}</div><div>120</div>

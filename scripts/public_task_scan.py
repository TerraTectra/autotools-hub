...
def fetch(url: str) -> str:
    request = urllib.request.Request(url, headers={"User-Agent": "TerraTectra-Task-Scanner/1.1"})
...
def age_hours(text: str) -> int | None:
    ...
    is_days = bool(re.search(r"\bдн", match.group(0), re.I) or re.search(r"\bдень\b", match.group(0), re.I))
    return value * 24 if is_days else value


def numbers_after_time(text: str) -> tuple[int | None, int | None]:
    match = TIME_RE.search(text)
    part = text[match.end():].split("Гонорар", 1)[0] if match else text
    part = re.sub(r"Опыт:\s*(?:Не важ(?:ен|на)|До 1 года|1[–-]3 года|От 3 лет)", " ", part, flags=re.I)
    raw = re.findall(r"(?<![\w№#])\d+(?:[\u00a0 ]\d{3})*(?![\w])", part)
    values = [int(re.sub(r"[\u00a0 ]", "", value)) for value in raw]
...
def main() -> int:
    ...
    cli.add_argument("output_dir", nargs="?", default="artifacts/public-task-scan")
    ...
    (output / "public-tasks.json").write_text(json.dumps(payload, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    ready = [item for item in items if item.actionable]
    lines = ["# Public task scan", "", f"Checked: **{len(items)}**, actionable: **{len(ready)}**, errors: **{len(errors)}**", ""]
    for item in items:
        lines.append(f"- {'READY' if item.actionable else 'skip'}: [{item.title}]({item.url}) — {', '.join(item.blockers) or 'manual review'}")
    (output / "public-tasks.md").write_text("\n".join(lines) + "\n", encoding="utf-8")
